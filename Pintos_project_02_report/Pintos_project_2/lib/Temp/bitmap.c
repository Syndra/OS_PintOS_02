...
bitmap_create (size_t bit_cnt)
{
  ...
  if (b != NULL)
    {
      b->bit_cnt = bit_cnt;
	  //(ADD) Initialize current starting index to 0.
	  b->cur_index = 0;
...
/* (ADD) Finds and returns the starting index of next group of CNT
	bit. */
size_t
bitmap_scan_nf(const struct bitmap *b, size_t start, size_t cnt, bool value)
{
	ASSERT(b != NULL);
	ASSERT(start <= b->bit_cnt);
...
bitmap_scan_bf(const struct bitmap *b, size_t start, size_t cnt, bool value)
{
	...
	if (cnt <= b->bit_cnt)
	{
		...
		for (i = start; i <= last; i++)
		{
			if (!bitmap_contains(b, i, cnt, !value))
			{
				for (j = i; !bitmap_test(b, j); j++)
				{
					size++;
					if (j+1 >= bit_cnt) break;
				}
				if (size < min_size)
				{
					min_index = i;
